...
@fig-1 exhibits the movement better. As we can see from the barplot that the proportion of `at-the-spread` crossings significantly decrease after May 2013. The proportion then dominated by `within-the-spread` crossings, as the rule amendment only allow meaningful price improvement on the dark pool. There were still some breach on the TWPI rule, however the number and proportion were not significant compared to the compliance dark trades.
