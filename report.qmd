...
To assess the economic implications, we evaluate the aggregate dollar value of `at-the-spread` trades over two periods: the 100 days preceding and the 100 days succeeding the rule amendment on 26 May 2013. This controlled period reflects similar transaction trends or patterns. The value-based analysis will measure the economic consequences of trades that may have shifted to the 'lit market' following the amendment, thus assessing the possible enhancement in market transparency and fairness.
...
@fig-prop exhibits the movement better. As we can see from the barplot that the proportion of `at-the-spread` crossings significantly decrease after May 2013. The proportion then dominated by `within-the-spread` crossings, as the rule amendment only allow meaningful price improvement on the dark pool. There were still some breach on the TWPI rule, however the number and proportion were not significant compared to the compliance dark trades.
...
combined |> 
  ...
  geom_vline(xintercept = as.Date("2013-05-01"), color = "red", linetype = "dashed", size = 1) +
  theme_bw() +
...
- compare the 100 days change in dollar value, before and after the rule change
- compare with whole at-or-within-spread dark trades change from CLA report, how much at-the-spread internal crossings compared to priority crossings
- compare with the whole transaction in the dark and lit market, from CLA report, how significant is the value?

## Violation Pattern After the Rule Change

- still violation, only around 5%, show barplot monthly
