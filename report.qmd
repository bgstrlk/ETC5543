...
This study use ASX transaction data and market data sourced from Securities Industry Research Centre of Asia-Pacific (SIRCA). SIRCA is a not-for-profit organization established in 1997, mainly consisting of Australian and New Zealand universities's academics. It has a number of financial market data from Australia, New Zealand, and even USA, including data from ASX. Members of SIRCA can collect the data freely for research purposes, baring the university they are in has to pay for subscription fee.

The ASX transaction data provided by SIRCA consists of the date and time, price, volume, dollar value, stock code, and also broker id. While the ASX market data provides the best bid and best offer order price, as well as the volume of those orders. This best bid and best offer price are used to calculate the spread of the market at each particular time. 

We use datasets from stocks listed on S&P/ASX200 index, as this index represents the group of stocks with the largest market capitalization. Meaning that these companies are the largest and the most valuable companies in the ASX. Market capitalization also often positively correlated with the liquidity. Both characteristics ensuring trading interest from large brokers and also ensure sufficient liquidity (Duong et al., 2018), which is important for measuring the impact of the rule change.

We will use the data from the inducement of `at or within the spread` rule in November 2011. However, as our investigation found that there were no internal crossings in the year 2011, we will use the data from 2012, until the of 2015. This ensure the similar length of the investigation period of before and after the rule change in 26 May 2013.

The transaction dataset we got from SIRCA filtered to include only internal crossing trades, marked as "OFFTR" in the `RecordType` variable and as "NX XT" in the `Qualifiers` variable. We combined the data with the market data, hence we got every transactions with the best bid and offer price and volume of the market. Combining these much data needs enormous processing power, hence we use the help of Nectar Research Cloud, cloud computing service from Australia Research Data Commons.

Methodology


- understand rule change impact: compare the proportion before and after at-the-spread crossings
- understand the economic worth of the dark trades: how much in dollar value at-the-spread crossings before and after the rule change, 30 days before and after to control for transaction trend increase
- understand violation pattern: using logistic regression and random forest
- with packages in R to do this
- statistics to be used: balanced accuracy, specificity, sensitivity
