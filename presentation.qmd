...
TWPI amendment in 2013 did significantly decrease the proportion of `at-the-spread` TWPI from 52% to 1.23%. However, the proportion of the violation rose from 1% to 4.5%.
::: 
:::
</div>
...
![](image/ASX.PNG){fig-align=center height="75%" width="75%"}
...
- Analyse violation pattern with other variables, such as stock's industry sector, market cap, lagged price values, bid-ask spread, etc.
- Try different method to analyse the pattern, such as PCA and factor modelling.
- More research questions:
    
    - How much profit did ASX make due to the rule amendment in May 2013?
    - Why there were only several brokers dominate the TWPI?
